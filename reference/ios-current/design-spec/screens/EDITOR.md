...
## 2026-08-17 iOS 3.11.52 갱신

- 퀵모드 추천시간은 실제 렌더링 화면당 1초, 최소시간은 화면당 0.1초다. 다중 영상의 선택된 중요 순간도 각각 한 화면으로 센다.
- 전체 시간을 각 화면에 균등 배분하되 짧은 영상·모션포토의 남는 시간은 다른 화면에 재배분한다.
- 만들기 버튼은 엔딩을 포함한 화면 수와 실제 예상 완성 시간을 표시한다.
- 저장된 퀵 프로젝트는 `클립 설정`의 `퀵모드` 표시판으로 길이 화면을 다시 연다.
- 제작 진행 미리보기는 최종 자막과 저작권 로고를 같은 위치에 표시한다.
- 엔딩 카드는 위치 정보가 없어도 촬영일 또는 현재 날짜로 만들며 정지 화면으로 합성한다.
- 기본 자막 위치는 `상단 가운데(upperCenter)`다.
