...
- 미검증: fontScale 1.6/2.0의 당일 변경분 사람 시각 AT, Fold 회전, TalkBack 실제 음성 탐색, 퀵 고정 헤더·안전영역 CTA의 사람 조작 및 iPhone 픽셀 동등성, 실제 콘텐츠가 필요한 트림·시사회·압축 P2 화면, 실기기 사진첩이 빈 상태라 실제 대량 사진의 첫 표시 시간·드래그 선형 범위·사진 선택 뒤 퀵 길이 화면으로 이어지는 마지막 단계. 사진 목록 선노출과 캐시 썸네일 경로는 정적 확인, 드래그는 JVM 시험, 마지막 단계는 선택 미디어 반영 뒤 기존 `LaunchedEffect`와 컴파일로 확인했으나 사람 AT로 완료 보고하지 않는다.
