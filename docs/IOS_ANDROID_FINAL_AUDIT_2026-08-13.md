...
- 직접 검증: `:app:testDebugUnitTest`, `:app:assembleDebug`, `git diff --check` 통과. `SM-F968N`에 `adb install -r` 성공, 설치 버전 `1.0.1 (530)` 확인, 홈 전체 문구와 여섯 프리셋 동일 크기·2줄 설명 노출 및 `홈 퀵모드 → 영화 제작 → 사진 자동 진입` 확인.
- 실기기 최종 홈 캡처: `/tmp/hanclip-home-527-top.png`. 빈 프로젝트를 저장하거나 사용자 콘텐츠를 삭제하지 않았고 `connectedDebugAndroidTest`는 실행하지 않았다.
- 미검증: fontScale 1.3/1.6/2.0의 당일 변경분 사람 시각 AT, Fold 펼침/회전의 당일 변경분, TalkBack 실제 음성 탐색, 퀵 고정 헤더·안전영역 CTA의 사람 조작 및 iPhone 픽셀 동등성, 실기기 사진첩이 빈 상태라 실제 대량 사진의 첫 표시 시간·드래그 선형 범위·사진 선택 뒤 퀵 길이 화면으로 이어지는 마지막 단계. 사진 목록 선노출과 캐시 썸네일 경로는 정적 확인, 드래그는 JVM 시험, 마지막 단계는 선택 미디어 반영 뒤 기존 `LaunchedEffect`와 컴파일로 확인했으나 사람 AT로 완료 보고하지 않는다.
