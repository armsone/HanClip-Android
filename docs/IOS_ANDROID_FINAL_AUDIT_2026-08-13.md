...
- 직접 검증: `:app:testDebugUnitTest`, `:app:assembleDebug`, `git diff --check` 통과. `SM-F968N`에 `adb install -r` 성공, 설치 버전 `1.0.1 (531)` 확인, 홈 전체 문구와 여섯 프리셋 동일 크기·80% 설명 노출, 1.3배 2열 적응 및 `홈 퀵모드 → 영화 제작 → 사진 자동 진입` 확인. 1.3배 시험 뒤 시스템 글자 배율은 원래 1.15로 복원했다.
- 실기기 최종 홈 캡처: `/tmp/hanclip-home-527-top.png`. 빈 프로젝트를 저장하거나 사용자 콘텐츠를 삭제하지 않았고 `connectedDebugAndroidTest`는 실행하지 않았다.
- 추가 실기기 확인: 펼침 상태 3의 2160×1584·320dpi에서 홈 920dp 중앙 폭, 3×2 프리셋과 2열 영화 목록이 잘리지 않았고 즉시 닫힘 상태 0으로 복원했다. 2.0배 시험은 화면이 자동 잠겨 검은 잠금 화면만 캡처되었으므로 성공으로 계산하지 않았고 글자 배율은 원래 1.15로 복원했다.
- 빌드 532에서 첨부 카드처럼 프리셋 내부를 동일한 세 수직 영역으로 분할해 아이콘·제목·설명을 각각 중앙 정렬했다. SM-F968N의 1.15배 3열과 1.3배 2열 캡처에서 동일 카드 크기, 균등 중심 간격과 문구 무잘림을 확인한 뒤 글자 배율을 1.15로 복원했다.
- 후속 빌드 540에서 실제 기기 미디어를 선택해 퀵 길이 화면까지 진입했다. 요약 세 행은 `항목 : 내용` 한 줄이며, 화면 비율은 비선택/선택 대비가 분명하고 만들기 버튼은 화면 비율 바로 아래에서 58dp 전체가 노출된다. 1.3배에서도 각각 화면 안에 유지됐고 시스템 글자 배율은 1.15로 복원했다.
- 후속 빌드 541에서 달력 `오늘`의 이동 후 두 번째 탭 전체 선택, `전날`의 무선택 시 어제 및 선택 시 가장 빠른 선택 날짜의 전날 전체 선택을 구현했다. 정책 시험 5건과 빌드가 통과했으며 SM-F968N에서 오늘 1개, 전날 24개, 그 전날 2개 선택을 확인했다.
- 남은 미검증: fontScale 1.6/2.0의 당일 변경분 사람 시각 AT, Fold 회전, TalkBack 실제 음성 탐색, iPhone 픽셀 동등성, 실제 콘텐츠가 필요한 트림·시사회·압축 P2 화면, 대량 사진의 정밀 첫 표시 시간과 드래그 실조작. 첫 목록은 실기기에서 명령 간격 1초 안에 보였으나 정밀 성능 측정으로 계산하지 않는다.

## 2026-08-14 퀵 저장 선행·Google 포토식 드래그 왕복

- 퀵모드와 일반 편집의 만들기 진입은 편집 가능한 현재 프로젝트를 먼저 저장하고, 저장 성공 뒤에만 완성본 제작과 시사회 이동을 시작한다. 따라서 퀵 완성 뒤 홈으로 돌아오면 영화 목록에서 다시 열 수 있고, 저장 실패 시 제작은 시작하지 않는다.
- 사진 선택은 제스처 시작 당시 선택 상태를 스냅숏으로 보존한 채 고정 시작 항목부터 현재 항목까지의 선형 범위를 매번 다시 적용한다. 손가락을 떼지 않고 반대 방향으로 이동하면 지나온 항목 중 현재 범위에서 빠진 항목이 시작 전 상태로 복원되어 선택·해제 양방향 왕복을 지원한다.
- 관련 정책 시험 8건과 빌드 542가 통과했다. SM-F968N에서 퀵모드→사진 자동 진입 뒤 한 제스처로 4칸 전진·2칸 후퇴했을 때 최종 2개만 선택되고 후퇴 범위 밖 2개가 해제되는 것을 접근성 트리로 확인했다. 선택은 취소했고 추가·저장·제작은 실행하지 않았다. 시사회 저장 선후관계는 사용자 영화 생성을 피하기 위해 JVM 정책 시험과 호출 순서로 검증했다.

## 2026-08-14 사진 하단 자동 스크롤 경계 보정

- 드래그가 실제로 시작되면 하단 `필터·전날·오늘·해제·추가` 막대를 숨기고 종료·취소 시 즉시 복원한다. 기존 고정 버튼이 그리드를 가리던 높이도 자동 스크롤 계산에 포함해 버튼 위치 전에 최고 하향 속도에 도달하게 했으며, 드래그가 화면 아래로 이어져도 미디어 판정 좌표는 마지막 비가림 그리드 위치로 제한하므로 스크롤과 연속 선택이 함께 진행된다.
- 상단 경계와 하단 버튼의 일반 탭 동작은 유지하고, 상·하단 최대 속도 및 하단 판정 좌표를 JVM 정책 시험으로 검증한다.
- SM-F968N 실기기에서 하향 드래그 중 하단 막대 전체 숨김, 자동 스크롤을 통한 39개 연속 선택, 손을 뗀 뒤 선택 유지와 막대 즉시 복원을 확인했다. 선택은 상단 취소로 폐기했으며 추가·저장·제작은 실행하지 않았다.

## 2026-08-14 GitHub Release 업데이트

- Android 앱 시작 시 공식 GitHub 저장소의 최신 안정 Release를 한 번 확인하고, 현재보다 높은 빌드만 선택형 안내창으로 표시한다. 다운로드와 설치 화면은 모두 사용자 동작 뒤에만 진행한다.
- Release 태그·APK 이름·HTTPS 경로·크기·package name·versionCode와 현재 설치 인증서 전체 일치를 검증한다. 실패 시 설치 화면을 열지 않으며 기존 영화·컬렉션·설정은 읽거나 변경하지 않는다.
- iOS에는 없는 Android 직접 배포 차이이며, 단순 소스 push는 업데이트 배포로 보지 않는다. 공개 Release와 낮은 설치본의 실제 업데이트 보존 시험은 `docs/GITHUB_UPDATES.md` 절차로 관리한다.
- 전체 JVM 시험, 디버그·계측 APK 컴파일, lint와 releaseQa 빌드가 통과했다. 디버그/releaseQa 인증서 SHA-256 일치, releaseQa `com.hanclip.android` 1.0.1(544), 설치 권한을 확인했다. SM-F968N과 Android 10 SM-T500에 데이터 보존 설치·실행했고 두 기기의 안정 Release JSON 계측시험 2건씩 통과했다. 실제 낮은 빌드→높은 Release 설치와 데이터 보존은 다음 높은 Release에서 최종 AT한다.
