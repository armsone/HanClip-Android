...
현재 Android 버전: `1.0.1` (`versionCode=434`)
...
- 홈 테마·레이아웃 동기화 후 빌드 422 releaseQa 1,277프레임 jank 4.39%(p50 5ms·p90 9ms·p95 10ms), 비트맵 업로드 0·PSS 132MB·RSS 266MB·비트맵 14.9MB 확인
- 저장 영상 오디오 분석을 iOS와 같은 시간 버킷 내림·말소리 억제 조건으로 맞추고 Android 디코더에 16비트 PCM을 요청하되 float PCM 반환도 정확히 분석
- 빌드 410에서 12.131초 오디오 포함 실영상의 192개 파형 버킷(최소 0.0946·최대 1.0)과 다중 타격점 생성을 확인하고 임시 프로젝트 제거
- 사진보다 내장 영상이 큰 Samsung Motion Photo도 MP4 시작점을 놓치지 않도록 전체 파일을 1MB 단위로 스트리밍 검색
- AiShot은 카메라 준비 직후 임시 영상을 연속 녹화하고 타격 시각 기준 앞·뒤 프리셋 구간만 MP4로 잘라 저장한 뒤 다음 감지 녹화를 자동 재시작
- AiShot 촬영 중 저장된 URI는 세션 안에서 순서를 유지해 모으고 편집 화면 진입 시 한 번에 가져와 비동기 처리 중 누락되지 않도록 함
- AiShot 감지 준비 시간을 iOS와 같은 선택 프리셋 앞 구간(1.5/2/5초)으로 바꾸고 준비 전후 기준음 적응률(0.04/0.012)과 말소리 오감지 억제 조건을 동기화
- 전체 화면 사진 시간·영상 구간 편집의 제목 영역이 닫기 버튼을 밀어내지 않도록 폭을 제한해 빌드 407에서 닫기 조작을 보강
- 실제 사진 1장 프로젝트로 홈 일반 영화 행과 사진 시간 편집을 확인한 뒤 테스트 프로젝트를 제거해 기존 사용자 목록을 복원
- 순서 변경 영역을 iOS의 4열 정방형 격자로 바꾸고 번호·미디어 유형·재생시간 배지와 Android용 앞/뒤/삭제 조작을 함께 배치해 빌드 406에서 실기기 확인
- Samsung Motion Photo를 사진 복사 없이 XMP 메타데이터로 분류해 `Live` 필터에 연결하고, 가져오기 시 전체 파일 메모리 적재 대신 스트리밍 MP4 추출로 변경; 현재 8월 기기 사진첩에는 실제 표본이 없어 빈 상태만 빌드 403에서 확인
- 사진 시간·영상 구간 편집을 iOS와 같은 전체 화면 흐름으로 전환하고 빌드 404에서 2.9초 영상의 미리보기·파형·타격점·시작/길이 조절 화면 확인
- iOS의 인스타그램·페이스북·유튜브·블로그·카카오톡·X·전화·홈페이지·사용자 이미지 원본 PNG를 Android 2×5 플랫폼 그리드에 재사용하고, 사용자 이미지 가져오기와 실제 아이콘+주소 MP4 비트맵 합성을 구현해 빌드 401 실제 렌더링에서 HanClip 이미지 워터마크 확인
- 시사회를 iOS처럼 HanClip 헤더·정방형 플레이어·`다시 편집/공유/개봉하기` 하단 조작으로 단순화하고, 개봉 위치를 `사진 앱/파일 앱` 전체 화면으로 맞춰 17.2초 실제 렌더링 뒤 빌드 400에서 확인
- 달력 가져오기를 iOS와 같은 전체 화면으로 전환하고 `취소/오늘/확인`, 원형 월 이동, `SUN~SAT` 색상 헤더, 7열 날짜 그리드를 실기기 빌드 396에서 확인
- 기본 사진첩 화면을 iOS와 같은 `취소–미디어 선택–추가`, `사진/Live/영상` 필터, 5열 미디어 그리드, 선택 순서 미리보기, `어제/오늘/해제` 구조로 바꾸고 빌드 397에서 선택 상태까지 확인
- iOS 실제 프로젝트 홈 기준(`IMG_6639.jpeg`)에 맞춰 AiShot 2열 카드와 일반 영화 행을 편집 가능한 프로젝트 중심으로 재구성하고 대표/클립 스트립 썸네일·클립 수·재생시간·표시 용량·편집·핀을 동기화
- iOS 영화 제작 화면처럼 기본시간·영상 길이·Live Photo·영상 분할·묶음사진·자막·음악을 하나의 압축 설정 표로 구성하고 화질·화면비·화면 유지·초기화는 접이식 고급 설정으로 이동
- 하단 제작 영역을 닫기 원형 버튼·화면비 선택 버튼·`재생시간 만들기` 캡슐 버튼으로 교정하고 `SM-F968N`에서 빌드 392 설치·실화면 확인
- 클립 행을 iOS처럼 번호·JPEG 썸네일·선택/전체 시간·핵심 모드·시간 증감만 보이도록 압축하고, 저장된 영상 프로젝트의 JPEG 썸네일을 이미지로 올바르게 디코딩하도록 수정한 뒤 빌드 395에서 실화면 확인
